...
static bool find_label(const RawInstruction *raws, size_t n, const char *name,
                       size_t *out_pc) {
  size_t pc = 0;
...
static size_t imm_to_usize(const ImmValue *v) {
  ...
  case IMM_INT_ZERO:
    return 0;
  case IMM_INT:
    return (size_t)(uint64_t)v->data.int_val;
  case IMM_F32_BITS:
    return (size_t)v->data.f32_bits;
  }
  return 0;
}
...
static InstInfo lower_st(const RawInstruction *r) {
  ...
  if (r->modifier_count == 2 && !strcmp(r->modifiers[0], "global")) {
    ...
    if (!strcmp(ty, "u32"))
      i.opcode = StGlobalU32;
    else if (!strcmp(ty, "f32"))
      i.opcode = StGlobalF32;
...
static size_t encode_src_operand(const RawInstruction *r,
                                 const RawOperand *src) {
  if (src->type == RAW_OP_REGISTER)
    return (size_t)src->data.reg.index;
  if (src->type == RAW_OP_IMMEDIATE)
    return imm_to_usize(&src->data.imm);
  return 0;
}
...
static InstType mov_opcode(const RawInstruction *r, const char *ty,
                           const RawOperand *src) {
  if (!strcmp(ty, "u32")) {
    if (src->type == RAW_OP_IMMEDIATE)
      return MovU32Imm;
...
static InstInfo lower_branch(const RawInstruction *r,
                             const RawInstruction *raws, size_t raw_count) {
  InstInfo i = {0};
  size_t pc;
...
// load logic
static bool find_param(const ParamInfo *params, size_t n, const char *name,
                       size_t *out_idx) {
  for (size_t i = 0; i < n; i++) {
...
static InstInfo lower_ld(const RawInstruction *r, const ParamInfo *params,
                         size_t param_count) {
  InstInfo i = {0};
  size_t dst = reg_index(r, &r->operands[0]);
  ...
  if (r->modifier_count == 2 && !strcmp(r->modifiers[0], "global")) {
    const char *ty = r->modifiers[1];
    if (!strcmp(ty, "u32"))
      i.opcode = LdGlobalU32;
    else if (!strcmp(ty, "f32"))
      i.opcode = LdGlobalF32;
  ...
  if (r->modifier_count == 3 && !strcmp(r->modifiers[0], "global") &&
      !strcmp(r->modifiers[1], "nc")) {
...
static InstInfo lower_mad(const RawInstruction *r) {
  ...
  const RawOperand *b_imm = &r->operands[2];
  const RawOperand *c = &r->operands[3];

  if (r->modifier_count == 2 && !strcmp(r->modifiers[0], "lo") &&
      !strcmp(r->modifiers[1], "s32")) {
    if (c->type == RAW_OP_IMMEDIATE) {
      i.opcode = MadLoS32Imm;
      i.args[0] = dst;
      i.args[1] = a;
      i.args[2] = imm_to_usize(&b_imm->data.imm);
...
static InstInfo lower_fma(const RawInstruction *r) {
  ...
  if (r->modifier_count == 2 && !strcmp(r->modifiers[1], "f32")) {
    if (!strcmp(r->modifiers[0], "rn"))
      i.opcode = FmaRnF32;
    else if (!strcmp(r->modifiers[0], "rm"))
      i.opcode = FmaRmF32;
...
static InstInfo lower_div(const RawInstruction *r) {
  ...
  if (r->modifier_count == 2 && !strcmp(r->modifiers[0], "rn") &&
      !strcmp(r->modifiers[1], "f32")) {
    i.opcode = DivRnF32;
    i.args[0] = dst;
    i.args[1] = a;
    i.args[2] = b;
...
static InstInfo lower_mul(const RawInstruction *r) {
  ...
  if (r->modifier_count == 2 && !strcmp(r->modifiers[0], "wide") &&
      !strcmp(r->modifiers[1], "s32")) {
    const RawOperand *b = &r->operands[2];
    i.opcode = MulWideS32;
    i.args[0] = dst;
...
static InstInfo lower_cvta(const RawInstruction *r) {
  ...
  if (r->modifier_count >= 2 && !strcmp(r->modifiers[0], "to") &&
      !strcmp(r->modifiers[1], "global")) {
...
static InstInfo lower_cvt(const RawInstruction *r) {
  ...
  if (r->modifier_count == 3 && !strcmp(r->modifiers[0], "sat") &&
      !strcmp(r->modifiers[1], "f32") && !strcmp(r->modifiers[2], "f32")) {
  ...
  if (r->modifier_count == 3 && !strcmp(r->modifiers[0], "rn") &&
      !strcmp(r->modifiers[1], "f32") && !strcmp(r->modifiers[2], "s32")) {
...
static InstInfo lower_ex2(const RawInstruction *r) {
  ...
  if (r->modifier_count == 2 && !strcmp(r->modifiers[0], "approx") &&
      !strcmp(r->modifiers[1], "f32")) {
...
static InstInfo lower_shr(const RawInstruction *r) {
  ...
  i.args[0] = dst;
  i.args[1] = a;
  i.args[2] = (b->type == RAW_OP_IMMEDIATE) ? imm_to_usize(&b->data.imm)
                                            : (size_t)b->data.reg.index;
  i.arg_count = 3;
...
static InstInfo lower_instruction(const RawInstruction *r,
                                  const RawInstruction *raws, size_t raw_count,
                                  const ParamInfo *params, size_t param_count) {

  if (!strcmp(r->instr_name, "ld"))
    return lower_ld(r, params, param_count);
  if (!strcmp(r->instr_name, "st"))
    return lower_st(r);
  if (!strcmp(r->instr_name, "fma"))
    return lower_fma(r);
  if (!strcmp(r->instr_name, "mad"))
    return lower_mad(r);
  if (!strcmp(r->instr_name, "add"))
    return lower_add(r);
  if (!strcmp(r->instr_name, "sub"))
    return lower_sub(r);
  if (!strcmp(r->instr_name, "mul"))
    return lower_mul(r);
  if (!strcmp(r->instr_name, "div"))
    return lower_div(r);
  if (!strcmp(r->instr_name, "neg"))
    return lower_neg(r);
  if (!strcmp(r->instr_name, "setp"))
    return lower_setp(r);
  if (!strcmp(r->instr_name, "and"))
    return lower_and(r);
  if (!strcmp(r->instr_name, "xor"))
    return lower_xor(r);
  if (!strcmp(r->instr_name, "or"))
    return lower_or(r);
  if (!strcmp(r->instr_name, "not"))
    return lower_not(r);
  if (!strcmp(r->instr_name, "shr"))
    return lower_shr(r);
  if (!strcmp(r->instr_name, "shl"))
    return lower_shl(r);
  if (!strcmp(r->instr_name, "cvta"))
    return lower_cvta(r);
  if (!strcmp(r->instr_name, "cvt"))
    return lower_cvt(r);
  if (!strcmp(r->instr_name, "ret"))
    return lower_ret(r);
  if (!strcmp(r->instr_name, "mov"))
    return lower_mov(r);
  if (!strcmp(r->instr_name, "bra"))
    return lower_branch(r, raws, raw_count);
...
ParsedKernel lower(ParseOutput parsed, Arena *arena) {
  ...
  for (size_t i = 0; i < parsed.raw_count; i++) {
    ...
    if (!strcmp(r->instr_name, ".label"))
      continue;

    out.instructions[out.instruction_count++] =
        lower_instruction(r, parsed.raw_instructions, parsed.raw_count,
                          parsed.params, parsed.param_count);
  }
  return out;
}
